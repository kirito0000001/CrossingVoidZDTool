...
> **还没接的部分**：同步到虚幻（`Content\GameActor2D\<角色>\Material\<动作>\`，
> 生成 `<动作>_Effect_FrameNN_Sprite` + `<动作>_Effect_Flipbook`，空帧 = Flipbook 关键帧的空对象，
> 并进第五步）。旧的 `ExAsset\Effect`（按"特效素材"分类同步过去的那条路）已经退役。
